...
def fixskew(img):
    pts1 = np.float32([[240,550],[1050,580],[0,1000],[1295,1000]])
    pts2 = np.float32([[0,0],[875,0],[0,460],[1000,460]])
...
while (cap.isOpened()):
    ...
    try:
        ...
        print("End of Video ", fileCounter, len(a))
        colorFlag = False
        if fileCounter < len(a)-1:
            cap.release()
            xy = formatxy(pinData)
            if len(xy) > 0:
                # Dontwant to insert rows for test
                print('xy pairs if', xy)
            ...
            cv2.imwrite('C:/DownloadsDP/Lane4Free/dpballgrayball'+ time.strftime("%Y%m%d") +'.jpg', img_gray_show)
            print('Saving line image ')
            # Don't delete temp files for testing
        ...
        if radius > 10:
            ...
            if oldxyData != None:
                if dist(oldxyData, xyData, 5):
                    ...
                    # cv2.circle(img_gray_show_line, oldxyData, 3, (0, 255, 0), -1)
                    cv2.circle(img_gray_show,center,int(radius),(0,255,0),2)
                    print('Old, new, radius', oldxyData,xyData,radius)
                # input("press enter")
            oldxyData = xyData
    ...
    cv2.putText(img_gray_show, a[fileCounter], (10,350), font, 1, (0, 255, 0), 2, cv2.LINE_AA)
    cv2.imwrite('C:/DownloadsDP/TempHold1/dpballgrayball'+ str(fileCounter) + time.strftime("%Y%m%d") +'.jpg', img_gray_show)
    cv2.imwrite('C:/DownloadsDP/TempHold1/dpballgrayball'+ str(fileCounter) + time.strftime("%Y%m%d") +'.jpg', img_gray_show_line)
    # if frameNo < 100:
